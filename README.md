...
本地前端 Demo 版「快记 + 安排」工作流。这是一个用于候选人笔试的移动端前端 Demo，目前已经迭代为功能较为完整的本地原型。
...
当前 Demo 版通过浏览器 localStorage 配置 LLM API，不提供界面填写入口。打开 DevTools（F12）→ Console，执行：

```js
localStorage.setItem("arkme-demo.llmConfig", JSON.stringify({
...
保存后刷新页面即可生效。支持任意 OpenAI-compatible `/chat/completions` 端点。

**注意**：部分 API（如 DeepSeek）不支持 `response_format: { type: "json_object" }`，适配器会自动降级到纯文本模式。

## 当前已实现

- 移动端主界面：快记首页、底部导航、侧边栏会话、我的页、设置页、洞见占位页
- 快记输入与发给自己
- AI 识别安排：快记通过 LLM 返回结构化 JSON，自动分流到时间轴或以后再说
- 安排时间轴视图：支持过去/今天/未来安排、详情编辑、完成、删除、移动到以后再说
- 以后再说：独立 Tab、列表、详情、删除、放回安排、完成、快捷时间操作和撤销
- 安排来源追溯：AI 创建的安排保留原始快记文本和识别结果
- AI 对话记录：Codex/Claude Code 协作记录同步到应用内会话页
- OpenAI-compatible Provider（Anthropic/Gemini/DeepSeek/通义千问为预留入口）
- 消息测试控制台：`/sendtest` 用于测试消息场景
...
- 多轮上下文合并、重复安排合并
- 普通用户的 API Key 设置界面
- Anthropic / Gemini / DeepSeek / 通义千问的真实适配器调用
- 群聊 / 私聊中的安排识别

## AI 自测用例

以下用例可用于验证 AI 安排识别是否正常工作：

| 序号 | 输入 | 预期结果 |
|------|------|----------|
| 1 | "明天下午3点去医院" | 创建到时间轴 |
| 2 | "昨天晚上那个酒店好吵啊，到了晚上9点还有人" | 不创建安排 |
| 3 | "明天上午记得问一下老师" | 创建到时间轴 |
| 4 | "我觉得以后还是要去剪个头发吧，找个时间去" | 创建到以后再说 |

## 交互设计说明

### 安排时间轴
- 使用纵向时间轴组织过去、今天和未来安排。
- 过去未完成安排弱化展示，避免用红色逾期提醒制造压力。
- 卡片支持完成、删除、移动到"以后再说"和详情编辑。
- 点击卡片进入详情，可编辑标题、时间、地点、关联人、备注，并查看来源。

### 以后再说
- "以后再说"不是删除，而是把暂时不想面对的安排从主时间轴中收起。
- 它承接模糊时间、无明确时间、用户主动搁置和较早过期的安排。
- 页面不采用时间轴，而是普通列表，避免变成第二个逾期任务列表。
- 左滑可以快捷选择今天 / 明天 / 选时间，将安排放回时间轴。
- 右滑删除，操作后支持撤销。
...
### 来源追溯
- AI 创建的安排会保留原始快记和识别结果。
- 在详情中可以查看"来源：快记"，帮助用户理解这条安排为什么被创建。

## 候选人答题流程

候选人克隆项目后，请用 Codex 或 Claude Code 客户端打开本项目，并先输入：
